...
export class GeminiWordGeneratorService {
 ...
 private url = 'https://script.google.com/macros/s/AKfycbyvNbZMdaZLe_1QgTwlCIp9nAOo_eZVX1OCYfFOKr2yJIivSDQxNupLEEuLEMa_oczgDg/exec';
  ...
  public async generateSentence(prompt:string,targetLanguage:string):Promise<string[]> {
    ...
    const result = await model.generateContent('Generate a sentence in'+targetLanguage +' using the word' +' '+prompt+'. And write that word at the end of the sentence the format of your anwser should be sentence.word');
    const generatedText = result.response.text();
    return generatedText.split('.').filter(word => word.trim() !== '');
  }  
